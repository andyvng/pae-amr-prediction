...
def main():
    ...
    if config['clustering_path']:
        cluster_df = pd.read_csv(config['clustering_path'])
        cluster_df = pd.get_dummies(cluster_df, columns=['cluster'])
        train_cluster = cluster_df.merge(pd.DataFrame({'id': train_ids}), on='id', how='inner').drop(labels=['id'], axis=1).to_numpy()
        test_cluster = cluster_df.merge(pd.DataFrame({'id': test_ids}), on='id', how='inner').drop(labels=['id'], axis=1).to_numpy()

        X_train = np.concatenate([X_train, train_cluster], axis=1)
        X_test  = np.concatenate([X_test, test_cluster], axis=1)
